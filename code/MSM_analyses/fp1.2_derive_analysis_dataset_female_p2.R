...
setwd("/mnt/bmh01-rds/vanstaa/practice2019/alex")
### SET THE ROOT DIRECTORY

### This is the second part of deriving the analysis dataset
### After doing the locf/nocb, there are still lots of variables with missing data, as it is missing at every follow up time
...
load("R_out_MSM2017/derive_analysis_dataset_female_p1.RData")
...
data_anal_predictor_imp <- subset(data_anal_predictor_imp, select = -c(Erec_dysfunc_tv))
...
data_anal_predictor_imp <- subset(data_anal_predictor_imp, 
                                  select = c(patid,pracid,index_date,gender,age_tv,Atrialfib_tv,Atypical_antipsy_med_tv,BMI_tv,
                                             ...
                                             T2dia_tv,Ethnicity_tv,FamHis_tv,HIV_tv,Hypertension_tv,
...
save.image("R_out_MSM2017/derive_analysis_dataset_female_p2.RData")
