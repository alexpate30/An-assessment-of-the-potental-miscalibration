...
for (i in (200000*(prog.num-1)+1):min(200000*prog.num,nrow(data_outcomes))){
  ...
  if ((data_outcomes[i,"CVD_cens"] == 0) & (data_outcomes[i,"CVD_time"] < 1827)){
    res_temp$event[nrow(res_temp)] <- 1} else if((data_outcomes[i,"CVD_cens"] == 1) & (data_outcomes[i,"CVD_time"] < 1827)){
...
save.image("R_out_MSM2017/derive_outcome_variables10_male.RData")
