...
## Before doing any stochastic imputation, want to remove the variable Erec_dysfunc from the female cohort so it odesn't get in the way
data_T0 <- subset(data_T0, select = -c(Erec_dysfunc_T0))
...
colnames(data_T0) <- c("patid","pracid","index_date_T0","gender","NelsonAalen","age","Atrialfib",
                       ...
                       "T2dia","Ethnicity","FamHis","HIV","Hypertension",
...
N.devel.female <- nrow(data_devel)
N.valid.female <- nrow(data_valid)
...
Anixety.prop<- c(100*prop.table(table(data_devel$Anxiety))[2],100*prop.table(table(data_valid$Anxiety))[2])
...
LVH.prop<- c(100*prop.table(table(data_devel$LVH))[2],100*prop.table(table(data_valid$LVH))[2])
...
rm(list=setdiff(ls(),list("N.devel.female","N.valid.female","cont.vars","cat.vars")))

save.image("R_out_MSM2017/baseline_table_female.RData")
