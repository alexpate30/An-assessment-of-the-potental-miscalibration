...
## Fit the model
fit_model <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1_T0 + BMIfrac1_T0 + BMIfrac2_T0 + SBPfrac1_T0 + SBPfrac2_T0 +
...
create_risks<-function(model,datain){
  ...
  C<-model$var
  S<-model.matrix(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1_T0 + BMIfrac1_T0 + BMIfrac2_T0 + SBPfrac1_T0 + SBPfrac2_T0 +
                    Atrialfib_T0 + Atypical_antipsy_med_T0 + Cholesterol_HDL_Ratio_T0 +
                    CKD345_T0 + Corticosteroid_use_T0 + T1dia_T0 + 
                    T2dia_T0 + Ethnicity_T0 + FamHis_T0 + HIV_T0 + Hypertension_T0 + 
...
plot.pred.devel.valid <- ggplot(plot.data.pred.devel.valid) + geom_point(aes(x=percentile,y=value,shape=variable)) + 
  scale_shape_manual(values=c(1,19)) + ggtitle("Female") + theme(legend.position = "bottom", legend.title = element_blank()) +
  xlab("10th of predicted risk") + scale_x_continuous(breaks=1:10) + ylab("Average 5 year risk")
...
save.image("R_out_MSM2017/analysis_calibration_before_after_2010_female.RData")


## Want to save the plots
ggsave("figures/MSM2017/standard.cox.pred.devel.valid.female.png",plot.pred.devel.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.valid.female.png",plot.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.devel.female.png",plot.devel,dpi=600)
