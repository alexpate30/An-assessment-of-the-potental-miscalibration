...
### --------------------------------------------------------------------------------------------------------------------------------------------
### This program does the first analysis, which is to develop a standard cox model using same criteria as QRISK3 in patients with index
### dates prior to 2010, and then check the calibration in patients with index dates after 2010.
### --------------------------------------------------------------------------------------------------------------------------------------------


## First load the imputed data, split into development and validation cohorts
load("R_out_MSM2017/standard_cox_datasets_fracpoly_male.RData")

## Set number of days at which we want to calulcate cvd risks
numdays <- 1826

## Fit the model
fit_model <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1_T0 + agefrac2_T0 + BMIfrac1_T0 + 
                     ...
                     Migraine_T0 + RA_T0 + Sev_men_ill_qrisk_T0 + SBP_var_T0 + Smoking_T0 + SLE_T0 + Townsend_T0, data=data_devel)
...
create_risks<-function(model,datain){
  ...
  C<-model$var
  S<-model.matrix(Surv(CVD_time,CVD_cens_R, type='right') ~ agefrac1_T0 + agefrac2_T0 + BMIfrac1_T0 + 
                    SBPfrac1_T0 + SBPfrac2_T0 +
                    Atrialfib_T0 + Atypical_antipsy_med_T0 + Cholesterol_HDL_Ratio_T0 +
                    CKD345_T0 + Corticosteroid_use_T0 + T1dia_T0 + 
                    T2dia_T0 + Erec_dysfunc_T0 + Ethnicity_T0 + FamHis_T0 + HIV_T0 + Hypertension_T0 + 
                    Migraine_T0 + RA_T0 + Sev_men_ill_qrisk_T0 + SBP_var_T0 + Smoking_T0 + SLE_T0 + Townsend_T0, data=datain)
  S<-S[,-1]
  p<-(S%*%B)[,1]
...
plot.devel <- ggplot(plot.data.devel) + geom_point(aes(x=percentile,y=value,shape=variable)) + scale_shape_manual(values=c(1,19)) + 
  ggtitle("Male - development cohort") + theme(legend.position = "bottom", legend.title = element_blank()) +
...
plot.valid <- ggplot(plot.data.valid) + geom_point(aes(x=percentile,y=value,shape=variable)) + scale_shape_manual(values=c(1,19)) + 
  ggtitle("Male - validation cohort") + theme(legend.position = "bottom", legend.title = element_blank()) +
  xlab("10th of predicted risk") + scale_x_continuous(breaks=1:10) + ylab("Average 5 year risk")
...
colnames(plot.data.pred.devel.valid)<-c("percentile","development cohort","validation cohort")
...
plot.pred.devel.valid <- ggplot(plot.data.pred.devel.valid) + geom_point(aes(x=percentile,y=value,shape=variable)) + 
  ...
  ggtitle("Male")+ theme(legend.position = "bottom", legend.title = element_blank()) +
  xlab("10th of predicted risk") + scale_x_continuous(breaks=1:10) + ylab("Average 5 year risk")
...
save.image("R_out_MSM2017/analysis_calibration_before_after_2010_male.RData")


## Want to save the plots
ggsave("figures/MSM2017/standard.cox.pred.devel.valid.male.png",plot.pred.devel.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.valid.male.png",plot.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.devel.male.png",plot.devel,dpi=600)
