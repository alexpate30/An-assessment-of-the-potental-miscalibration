...
## Fit the model
fit_model <- coxph(Surv(CVD_time,CVD_cens_R, type='right') ~ c.timefrac1_T0 + c.timefrac2_T0 + agefrac1_T0 + agefrac2_T0 + BMIfrac1_T0 + 
...
create_risks<-function(model,datain){
  ...
  C<-model$var
  S<-model.matrix(Surv(CVD_time,CVD_cens_R, type='right') ~ c.timefrac1_T0 + c.timefrac2_T0 + agefrac1_T0 + agefrac2_T0 + BMIfrac1_T0 + 
                    SBPfrac1_T0 + SBPfrac2_T0 +
                    Atrialfib_T0 + Atypical_antipsy_med_T0 + Cholesterol_HDL_Ratio_T0 +
                    CKD345_T0 + Corticosteroid_use_T0 + T1dia_T0 + 
                    T2dia_T0 + Erec_dysfunc_T0 + Ethnicity_T0 + FamHis_T0 + HIV_T0 + Hypertension_T0 + 
...
plot.pred.devel.valid <- ggplot(plot.data.pred.devel.valid) + geom_point(aes(x=percentile,y=value,shape=variable)) + 
  scale_shape_manual(values=c(1,19)) + ggtitle("Male - 5 year risk") + 
...
save.image("R_out_MSM2017/analysis_calibration_before_after_2010_timeadj_male.RData")


## Want to save the plots
ggsave("figures/MSM2017/standard.cox.pred.devel.valid.timeadj.male.png",plot.pred.devel.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.valid.timeadj.male.png",plot.valid,dpi=600)
ggsave("figures/MSM2017/standard.cox.calibration.devel.timeadj.male.png",plot.devel,dpi=600)
